...
class POS_NER_Vocab(object):
  """POSVocabulary class for mapping between pos_tags and ids (integers)"""

  def __init__(self, pos_vocab_file, max_size):
    self._pos_to_id = {}
    self._id_to_pos = {}
    self._count = 0 # keeps track of total number of words in the Vocab

    # [UNK], [PAD] get the ids 0,1.
    ...
    with open(pos_vocab_file, 'r') as vocab_f:
      for line in vocab_f:
        pieces = line.split()
        if len(pieces) != 2:
          print 'Warning: incorrectly formatted line in pos_vocabulary file: %s\n' % line
          continue
        w = pieces[0]
        if w in [SENTENCE_START, SENTENCE_END, UNKNOWN_TOKEN, PAD_TOKEN, START_DECODING, STOP_DECODING]:
          raise Exception('<s>, </s>, [UNK], [PAD], [START] and [STOP] shouldn\'t be in the vocab file, but %s is' % w)
    ...
    print "Finished constructing pos_vocabulary of %i total words. Last word added: %s" % (self._count, self._id_to_pos[self._count - 1])

  def word2id(self, word):
    """Returns the id (integer) of a word (string). Returns [UNK] id if word is OOV."""
    if word not in self._pos_to_id:
      return self._pos_to_id[UNKNOWN_TOKEN]
    return self._pos_to_id[word]

  def id2word(self, word_id):
    """Returns the word (string) corresponding to an id (integer)."""
    if word_id not in self._id_to_pos:
      raise ValueError('Id not found in pos_vocab: %d' % word_id)
    return self._id_to_pos[word_id]
