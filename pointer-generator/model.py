...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    self._enc_batch_ner = tf.placeholder(tf.int32, [hps.batch_size, None], name='enc_batch_ner')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    feed_dict[self._enc_batch_ner] = batch.enc_batch_ner
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        ...
        if FLAGS.pos_ner:
        	# pos_embedding/ner_embedding
        	pos_embedding = tf.get_variable('pos_embedding', [FLAGS.pos_vocab_size, hps.pos_emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init)
        	ner_embedding = tf.get_variable('ner_embedding', [FLAGS.ner_vocab_size, hps.ner_emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init)

        if hps.mode=="train": self._add_emb_vis(embedding) # add to tensorboard
        emb_enc_inputs = tf.nn.embedding_lookup(embedding, self._enc_batch) # tensor with shape (batch_size, max_enc_steps, emb_size)
        if FLAGS.pos_ner:
	        # extra embeddings
	        emb_enc_inputs_pos = tf.nn.embedding_lookup(pos_embedding, self._enc_batch_pos)
	        emb_enc_inputs_ner = tf.nn.embedding_lookup(ner_embedding, self._enc_batch_ner)
	        # final emb_enc_inputs
	        emb_enc_inputs = tf.concat(axis=2, values=[emb_enc_inputs, emb_enc_inputs_pos, emb_enc_inputs_ner])
